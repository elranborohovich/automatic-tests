"""
בדיקות של: יעקב אדלר - Mobile
4 בדיקות UI + 2 בדיקות API
מיקוד: חנות, עגלת קניות, תשלום (E2E), Footer.
סביבה: Samsung Galaxy S24 (mobile emulation)
"""
import os
import re
import pytest
...
USER_EMAIL = os.environ.get("USER_EMAIL", "private.user@gmail.com")
...
def _clear_cart(page: Page) -> None:
    """מרוקן את העגלה בשרת לפני/אחרי בדיקה."""
...
@pytest.fixture
def mobile_page(browser: Browser, playwright: Playwright):
    """Fixture המדמה Samsung Galaxy S24."""
...
def test_mobile_add_product_to_cart_updates_counter(mobile_page: Page):
    page = mobile_page
...
def test_mobile_cart_quantity_and_total_recalculation(mobile_page: Page):
    page = mobile_page
    page.goto(f"{BASE_URL}/pages/store.html", wait_until="networkidle")
    page.wait_for_timeout(1500)
    page.locator("[data-test='btn-add-cup']").click()
...
def test_mobile_payment_empty_required_fields_blocked(mobile_page: Page):
    page = mobile_page
    page.goto(f"{BASE_URL}/pages/store.html", wait_until="networkidle")
    page.wait_for_timeout(1500)
    page.locator("[data-test='btn-add-cup']").click()
...
def test_mobile_footer_content_and_links(mobile_page: Page):
    page = mobile_page
    page.goto(f"{BASE_URL}/pages/home.html", wait_until="networkidle")
    page.wait_for_timeout(1500)
...
def test_mobile_get_single_recommendation_200_and_404():
...
def test_mobile_delete_recommendation_permissions():
    user_token = _api_login(USER_EMAIL, USER_PASSWORD)
    admin_token = _api_login(ADMIN_EMAIL, ADMIN_PASSWORD)

    create = requests.post(
        f"{BASE_URL}/api/recommendations",
        data={"category": "Movie", "name": "Test Delete Permissions Mobile", "recommender_name": "Admin Tester"},
